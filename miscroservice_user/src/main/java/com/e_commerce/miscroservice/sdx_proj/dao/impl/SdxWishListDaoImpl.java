...
public class SdxWishListDaoImpl implements SdxWishListDao {
	...
	public List<TSdxWishListPo> findTSdxWishListByAll(TSdxWishListPo tSdxWishListPo, Integer page, Integer size) {
		...
		if (tSdxWishListPo.getId() == null) {
			...
			if (tSdxWishListPo.getUserId() != null) {
				build.eq(TSdxWishListPo::getUserId, tSdxWishListPo.getUserId());
			}
			if (page == null) {
				page = 1;
			}
	...
	@Override
	public TSdxWishListPo selectByUserIdAndBookInfoId(Long userId, Long bookInfoId) {
		return MybatisPlus.getInstance().findOne(new TSdxWishListPo(), baseBuild()
			.eq(TSdxWishListPo::getUserId, userId)
			.eq(TSdxWishListPo::getBookInfoId, bookInfoId)
		);
	}
}
